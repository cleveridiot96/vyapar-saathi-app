...
import { PlusCircle, ArrowRightLeft, ListChecks, Boxes, Printer, MoreVertical, Edit, Trash2, Download } from "lucide-react";
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    {expandedTransfers.map(transfer => {
                       const stockItem = availableStock.find(s => s.lotNumber === transfer.item.originalLotNumber);
                       const perKgExpense = (transfer.totalTransferCost && transfer.item.netWeight > 0) ? transfer.totalTransferCost / transfer.item.netWeight : 0;
                       const finalLandedCost = (stockItem?.effectiveRate || 0) + perKgExpense;
                      ...
                      <TableRow key={`${transfer.id}${KEY_SEPARATOR}${transfer.item.originalLotNumber}`} className="uppercase">
                        ...
                        <TableCell><Tooltip><TooltipTrigger asChild><span className="truncate max-w-[150px] inline-block">{transfer.fromLocationName}</span></TooltipTrigger><TooltipContent><p>{transfer.fromLocationName}</p></TooltipContent></Tooltip></TableCell>
                        <TableCell><Tooltip><TooltipTrigger asChild><span className="truncate max-w-[150px] inline-block">{transfer.toLocationName}</span></TooltipTrigger><TooltipContent><p>{transfer.toLocationName}</p></TooltipContent></Tooltip></TableCell>
                        <TableCell>
